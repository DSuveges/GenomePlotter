...
def main(cfg: DictConfig) -> None:
    ...
    if not os.path.isdir(cfg.shared_parameters.source_data_folder):
        raise ValueError(
            f"The provided folder ({cfg.shared_parameters.source_data_folder}) does not exist"
        )

    logging.info(
        f"Pre-processed data is saved to {cfg.shared_parameters.source_data_folder}"
    )
    ...
    configuration["source_data"]["gwas_data"][
        "release_date"
    ] = gwas_retrieve.get_release_date()
    ...
    configuration["source_data"]["cytoband_data"][
        "genome_build"
    ] = cytoband_retrieve.get_assembly_build()
    ...
    configuration["source_data"]["gencode_data"][
        "release_date"
    ] = gencode_retrieve.get_release_date()
    configuration["source_data"]["gencode_data"][
        "version"
    ] = gencode_retrieve.get_release()
    logging.info(
        f"Saving processed data: {configuration['source_data']['gencode_data']['processed_file']}."
    )
    ...
    # Save config file:
    logging.info(
        f"Saving updated configuration as {config_file.replace('json', 'updated.json')}"
    )
