...
class gwas_annotator(object):
    ...
    def __init__(self, pixel, chromosome, gwas_file, chunk_size, width, xoffset=0, yoffset=0, gwas_color="black"):
        """
        This class generates gwas signals based on the provided parameters
        """

        # Reading gwas file:
        gwas_df = pd.read_csv(gwas_file, compression='gzip', sep='\t', quotechar='"', header=0,
        ...
        data = []
        for index, value in filtered_locations.start.apply(lambda x: int(x / chunk_size)).value_counts().iteritems():
    ...
    def generate_gwas(self):
        ...
        for _, row in positions.iterrows():
            ...
            # Adding point{}
            gwas_points.append(self.gwas_hit.format(
