...
class chromosome_plotter(object):
    ...
    def __add_centromere(self):
        ...
        centromere_start = (
            self.__chromosomeData__.loc[self.__chromosomeData__.GENCODE == 'centromere'].y.min() * self.__pixel__
        )
        centromere_end = (
            self.__chromosomeData__.loc[self.__chromosomeData__.GENCODE == 'centromere'].y.max() * self.__pixel__
        )
    ...
    def draw_dummy(self):
        ...
        centromere_color = (
            self.__chromosomeData__.loc[self.__chromosomeData__.GENCODE == 'centromere'].color.tolist()[0]
        )

        # Extract centromere positions:
        centromere_start = (
            self.__chromosomeData__.loc[self.__chromosomeData__.GENCODE == 'centromere'].y.min()
            * self.__pixel__
        )
        centromere_end = (
            self.__chromosomeData__.loc[self.__chromosomeData__.GENCODE == 'centromere'].y.max()
        ...
        self.__plotString__ += self.chunk_svg.format(
    ...
    def wrap_svg(self, fileName):
        self.__svg__ = (
            '<svg width="%s" height="%s" version="1.1" xmlns="http://www.w3.org/2000/svg" \
                ...
                self.__width__, self.__height__, self.__plotString__
            )
        )
        f = open(fileName, 'w')
