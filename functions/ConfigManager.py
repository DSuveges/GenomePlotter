    ...
    * Upon retrieveal file and folder configuration, check performed for existence
    ...
    def __init__(self, config_file_name):
        ...
        if not os.path.isfile(config_file_name):
            raise ValueError(
                f"The provided config file ({config_file_name}) doesn't exits."
            )
    ...
    def get_source(self, resource, key, chromosome=None):
        ...
        if resource not in self.__sources:
            ...
            raise ValueError(
                f"Unknown resource presented: {resource}. Accepted resources: {accepted_resources}"
            )

        if key not in self.__sources[resource]:
            available_features = ",".join(self.__sources[resource].keys())
            raise ValueError(
                f"{key} is not a stored feature for {resource}. Available features: {available_features}"
            )
